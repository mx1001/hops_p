...
class HopsBoolDifference(bpy.types.Operator):
    ...
    def execute(self, context):
        if self.boolean_method == 'BMESH':
            use_bmesh_boolean(context, 'DIFFERENCE')
        elif self.boolean_method == 'CARVE':
            use_carve_boolean(context, 'DIFFERENCE')
        elif self.boolean_method == 'CARVEMOD':
            use_carve_mod_boolean(context, 'DIFFERENCE')
        elif self.boolean_method == 'BMESHMOD':
            use_bmesh_mod_boolean(context, 'DIFFERENCE')
...
def draw(display, parameter_getter):
    ...
    # Name
    ########################################################
    draw_text("DIFFERENCE", x - 380 *dpi_factor , y -12*dpi_factor,
