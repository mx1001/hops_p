...
class HopsBoolIntersect(bpy.types.Operator):
    ...
    def execute(self, context):
        if self.boolean_method == 'BMESH':
            use_bmesh_boolean(context, 'INTERSECT')
        elif self.boolean_method == 'CARVE':
            use_carve_boolean(context, 'INTERSECT')
        elif self.boolean_method == 'CARVEMOD':
            use_carve_mod_boolean(context, 'INTERSECT')
        elif self.boolean_method == 'BMESHMOD':
            use_bmesh_mod_boolean(context, 'INTERSECT')
...
def draw(display, parameter_getter):
    ...
    # Name
    ########################################################
    draw_text("INTERSECT", x - 380 *dpi_factor , y -12*dpi_factor,
